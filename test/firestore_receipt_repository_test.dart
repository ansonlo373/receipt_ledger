...
void main() {
  ...
  test('add returns the new id so the caller can attach a photo to it', () async {
  ...
  test('add and update both store a photoPath that watchAll reflects', () async {
    ...
    expect((await repository.watchAll().first).single.photoPath,
        '/receipts/photo1.jpg');
  ...
  test('softDelete removes a receipt from watchAll but keeps it in watchTrash',
      () async {
    final id = await repository.add(
      merchant: 'Trader Joe\'s',
      amountYen: 4599,
  ...
  test('purgeExpiredTrash removes only trash older than the retention window',
      () async {
