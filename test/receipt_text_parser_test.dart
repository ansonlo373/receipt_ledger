...
void main() {
  ...
  test('prefers the amount on a line with a total keyword over other numbers', () {
    final result = parseReceiptText(
      'ローソン\n'
      'おにぎり 150\n'
      'お茶 120\n'
  ...
  test('falls back to the largest number when no total keyword line exists', () {
    final result = parseReceiptText('Store\n120\n45\n300');
  ...
  test('handles a stray space after the thousands comma (real OCR artifact)', () {
  ...
  test(
    'prefers a repeated yen amount even when comma-space OCR artifacts are present',
    () {
      final result = parseReceiptText(
        'Store\n'
        '登録番号 T8011503002037\n'
  ...
  test(
    'trusts a comma-grouped number on the keyword row even without a '
  ...
  test(
    'falls back to a repeated currency-marked amount when the keyword row '
  ...
  test('includes comma-grouped amounts without a currency marker as candidates', () {
    final result = parseReceiptText('Store\n1,628\n8%\n6点');
  ...
  test('tolerates stray whitespace around date separators (row-merge artifact)', () {
